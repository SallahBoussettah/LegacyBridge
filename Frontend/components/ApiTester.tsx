...
interface ApiTesterProps {
  config: Omit<ApiEndpoint, "id" | "status"> | ApiEndpoint;
  onClose: () => void;
}
...
export const ApiTester: React.FC<ApiTesterProps> = ({ config, onClose }) => {
  ...
  const generateCurlCommand = () => {
    ...
    if (
      ...
      const bodyParams = config.parameters.filter(
        (param) => !config.path.includes(`{${param.name}}`)
      );
  ...
  const simulateApiTest = async () => {
    ...
    try {
      // Check if we have an endpoint ID (saved endpoint) or if it's just a generated config
      if ("id" in config && config.id) {
        // This is a saved endpoint, use the real backend test
        const response = await apiService.testApiEndpoint(
          config.id,
          parameters
        );
        ...
        if (response.success && response.data) {
          setTestResult({
            ...
            headers: {
              ...
              "X-Execution-Mode": response.data.isMock ? "mock" : "database",
              "X-Response-Time": `${response.data.executionTime || 0}ms`,
              ...(response.data.truncated && { "X-Result-Truncated": "true" }),
            },
            duration: response.data.executionTime || 0,
          });
        } else {
          setError(response.message || "Failed to test API endpoint");
        }
      } else {
        // This is a generated config that hasn't been saved yet
        // Use mock data based on execution mode
      ...
      case "GET":
        return {
          ...
          headers: {
            ...
            "X-Mock-Response": executionMode === "mock" ? "true" : "false",
          },
  ...
  const generateMockData = () => {
    ...
    // Generate different mock data based on execution mode
    if (executionMode === "database") {
      // More realistic data that looks like it came from a database
      mockData._note = "This would be real data from your database";
      mockData._executionMode = "database";
    } else {
      // Generic mock data
      mockData._note = "This is mock data for testing";
      mockData._executionMode = "mock";
    }
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-600 mt-1">
              Test your API endpoint with sample data
            </p>
          </div>
          <Button
            variant="secondary"
      ...
      <CardContent className="space-y-6">
        ...
        <div className="flex items-center justify-center">
          <Button
            ...
            disabled={isLoading}
            leftIcon={
              isLoading ? <div className="loading-spinner" /> : <PlayIcon />
            }
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      copyToClipboard(JSON.stringify(testResult.data, null, 2))
                    }
                    leftIcon={<CopyIcon />}
                  >
                    Copy JSON
        ...
        >
          <div className="flex">
            ...
            <div className="ml-3">
              ...
              >
                {config.executionMode === "database"
                  ? "This endpoint will execute against your real database when saved and deployed."
